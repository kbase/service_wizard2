...
class CachedCatalogClient:
    ...
    def get_module_info(self, module_name, version, require_dynamic_service=False) -> dict:
        ...
        if require_dynamic_service:
            if "dynamic_service" not in module_info:
                raise ValueError("Specified module is not marked as a dynamic service. (" + module_info["module_name"] + "-" + module_info[
                    "git_commit_hash"] + ")")
    ...
    def get_hash_to_name_mappings(self):
        ...
        module_hash_mapppings = self.module_hash_mapppings_cache.get(key=key, default=None)
        if not module_hash_mapppings:
            ...
            for m in basic_module_info:
                ...
                if "dynamic_service" not in m or m["dynamic_service"] != 1:
                    continue
                module_hash_mapping[get_module_name_hash(m["module_name"])] = m["module_name"]

            self.module_hash_mapppings_cache.set(key=key, value=module_hash_mapping)
