...
async def start(request: Request, params: list[dict], jrpc_id: int) -> JSONRPCResponse:
    ...
    try:
        deployment_status = start_deployment(request, module_name, module_version)
        return JSONRPCResponse(id=jrpc_id, result=[deployment_status])
...
async def stop(request: Request, params: list[dict], jrpc_id: int) -> JSONRPCResponse:
    """
    Start a service
    """
    module_name = params[0].get("module_name")
    ...
    try:
        deployment_status = stop_deployment(request, module_name, module_version)
        return JSONRPCResponse(id=jrpc_id, result=[deployment_status])
    except ServerError as e:
        traceback_str = traceback.format_exc()
        return JSONRPCResponse(id=jrpc_id, error=ErrorResponse(message=f"{e.message}", code=-32000, name="Server error", error=f"{traceback_str}"))
