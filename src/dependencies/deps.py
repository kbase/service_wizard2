...
async def authenticated_user(
    authorization: str = Header(None, regex=ALPHANUMERIC_PATTERN, alias="Authorization", description="KBase auth token"),
    ...
    if not authorization and not kbase_session_cookie:
        raise HTTPException(status_code=400, detail="Please provide the 'Authorization' header or 'kbase_session_cookie'")
    ...
    try:
        check_or_cache_token(token=authorization if authorization else kbase_session_cookie, token_cache=token_cache)
