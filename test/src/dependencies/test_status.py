...
def test_get_service_status_without_retries_fallback_path(
    mock_get_with_retries, mock_fallback_helper, mock_request
):
    """
    ...
    mock_get_with_retries.side_effect = Exception(
        "Failed to get service status after maximum retries"
    )

    # Call the function you are testing
    get_service_status_without_retries(
        mock_request, sample_module_name, sample_git_commit
    )

    # Assert that the initial attempt was made
    mock_get_with_retries.assert_called_once_with(
        mock_request, sample_module_name, sample_git_commit, retries=1
    )

    # Assert that the fallback function was called as a result
    mock_fallback_helper.assert_called_once_with(
        mock_request, sample_module_name, sample_git_commit
    )
